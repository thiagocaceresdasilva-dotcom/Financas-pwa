...
async function renderMonth(){
  ...
  const pInc = sumBy(planned, "income") + sumBy(settled, "income");
  ...
  for (const r of ordered){
    ...
    li.innerHTML = `
      ...
      <div style="display:flex;gap:10px;align-items:center;">
        ...
        <button class="btn btn--ghost" data-toggle="${r.id}" type="button">${r.isActive ? "Desativar" : "Ativar"}</button>
        <button class="btnDanger" data-del-rule="${r.id}" type="button">X</button>
      </div>
    `;
...
entryForm.addEventListener("submit", async (ev) => {
  ...
  const e = {
    ...
    status: entryStatus.value, // planned | settled
    date,
...
(function setupReceiptOCR(){
  ...
  scanBtn.onclick = async () => {
    ...
    if (!file) return alert("Selecione ou tire a foto do cupom.");

    preview.src = URL.createObjectURL(file);
    preview.style.display = "block";

    if (!window.Tesseract) {
      statusEl.textContent = "OCR não carregou. Abra com internet e tente de novo.";
      return;
    }

    statusEl.textContent = "Lendo cupom (OCR)...";
    ...
    try{
      ...
      const total = extractTotalBRL(text);
      const dateISO = extractDateISO(text);
      const store = extractStore(text);

      // define automaticamente como despesa realizada
      entryType.value = "expense";
      entryStatus.value = "settled";
      renderCategorySelects(); // troca lista de categorias p/ despesas

      if (total != null) entryAmount.value = total.toFixed(2);
      if (dateISO) entryDate.value = dateISO;
      if (store) entryNote.value = store;

      statusEl.textContent = "Pronto! Confira e toque em Salvar.";
    } catch(e){
      console.error(e);
      statusEl.textContent = "Falhou ler o cupom. Tente com mais luz e foco no TOTAL.";
      alert("Falhou ler o cupom. Faça uma foto mais nítida do TOTAL.");
    }
  };

  function extractTotalBRL(t){
    // pega valores tipo 1.234,56 e escolhe o maior (normalmente é o total)
    const matches = [...t.matchAll(/(\d{1,3}(?:\.\d{3})*,\d{2})/g)].map(m => m[1]);
    const nums = matches
      .map(s => Number(s.replace(/\./g,"").replace(",", ".")))
      .filter(n => Number.isFinite(n) && n > 0);
    return nums.length ? Math.max(...nums) : null;
  }
  function extractDateISO(t){
    const m = t.match(/(\d{2})[\/\-\.](\d{2})[\/\-\.](\d{4})/);
    if (!m) return "";
    return `${m[3]}-${m[2]}-${m[1]}`;
  }
  function extractStore(t){
    // heurística simples: primeiras palavras (funciona bem em muitos cupons)
    const first = t.split(" ").slice(0, 6).join(" ").trim();
